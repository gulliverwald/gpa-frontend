...
const newsReducerSlice = createSlice({
  ...
  reducers: {
    requestCreateNewsSuccess(
      state,
      action: PayloadAction<IRequestCreateNewsSucess>,
    ) {
    ...
    requestListNewsSuccess(
      state,
      action: PayloadAction<IRequestListNewsSucess>,
    ) {
      const { payload } = action;
    ...
    requestUpdateNewsSuccess(
      state,
      action: PayloadAction<IRequestUpdateNewsSucess>,
    ) {
    ...
    requestDeleteNewsSuccess(
      state,
      action: PayloadAction<IRequestDeleteNewsSucess>,
    ) {
      const index = state.news.findIndex(news => news.id === action.payload.id);
...
export const requestCreateNews = createAction<
  IRequestCreateNews,
  '@news/requestCreateNews'
>('@news/requestCreateNews');

export const requestListNews = createAction<
  IRequestListNews,
  '@news/requestListNews'
>('@news/requestListNews');

export const requestDeleteNews = createAction<
  IRequestDeleteNews,
  '@news/requestDeleteNews'
>('@news/requestDeleteNews');

export const requestUpdateNews = createAction<
  IRequestUpdateNews,
